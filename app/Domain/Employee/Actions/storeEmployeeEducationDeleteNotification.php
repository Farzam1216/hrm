...
class storeEmployeeEducationDeleteNotification
{
    public function execute($request,$id)
    {
        $previousData = Education::where('id',$request->id)->with('EducationType')->first();
        $previousInfo = Employee::find($previousData->employee_id);
        ...
        if($previousInfo->id == Auth::user()->id){
            $message =  "$previousInfo->firstname $previousInfo->lastname deleted  $gender  education $status.";
        }else{
            $userName = Auth::user()->fullname;
            $message =  "$userName deleted $previousInfo->fullname education $status.";
        }
