...
class storeEmployeeNewEmploymentStatusNotification
{
    public function execute($request,$id)
    {
        ...
        if($previousInfo->id == Auth::user()->id){
            $message =  "$previousInfo->firstname $previousInfo->lastname added  $gender new employment status.";
        }else{
            $userName = Auth::user()->fullname;
            $message =  "$userName added $previousInfo->fullname new employment status.";
        }
