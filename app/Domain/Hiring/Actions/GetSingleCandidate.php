...
class GetSingleCandidate
{
    ...
    {
        $count = 0;
        $candidate['candidate'] = Candidate::with('answers', 'job')
            ->where('id', $id)
            ->first();
        $candidate['emails'] = CandidateEmail::where('can_id', $candidate['candidate']->id)->with('candidateEmailAttachments','emailTemplate.emailAttachments')->orderBy('created_at', 'DESC')
        ->get();
        $created = new Carbon($candidate['candidate']->created_at);
        $now = Carbon::now();
        $candidate['difference'] = ($created->diffForHumans($now));
        $candidate['status'] = CandidateStatus::with('comment')
        ->where('candidate_id', $id)
        ->orderBy('created_at', 'DESC')
            ->get();
        $candidate['currentStatus'] = CandidateStatus::with('comment')
        ->where(['candidate_id' => $id, 'change_in' => 'Status'])
        ->orderBy('created_at', 'DESC')
            ->first();
        $candidate['currentRating'] = CandidateStatus::with('comment')
        ->where(['candidate_id' => $id, 'change_in' => 'Rating'])
        ->orderBy('created_at', 'DESC')
            ->first();
        $candidate['answers'] = CandidateAnswers::with('jobQuestion', 'candidate', 'jobQuestion.question')
        ->whereHas(
            'candidate',
            function ($can_id) use ($id) {
                $can_id->where('id', $id);
            }
        )
            ->get();

        $candidate['jobQuestionsHistory'] = DB::table('job_question_histories')
        ->orderBY('jobque_id', 'asc')
            ->orderBy('created_at', 'desc')
            ->get();
        $candidate['questionsHistory'] = DB::table('question_histories')
        ->orderBY('que_id', 'asc')
            ->orderBy('created_at', 'desc')
            ->get();
        $answers = null;
