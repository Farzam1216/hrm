...
class StoreEmploymentStatusUpdateNotification
{
    public function execute($request,$id)
    {
        ...
        $differenceArray = array_diff($data,EmploymentStatus::where('id',$id)->first()->toArray());
        ...
        $message =  "$userName updated employment status ($request->employment_status) ";
        $showMessage = $message.$changedText;
        $showMessage= str_replace("_method , ", "",  $showMessage);
        $showMessage= str_replace("_token , ", "",  $showMessage);
        $showMessage= str_replace("employment_status", "name",  $showMessage);
        $title = "Update Employment Status Notification";
