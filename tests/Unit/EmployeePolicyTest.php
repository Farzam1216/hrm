...
class EmployeePolicyTest extends TestCase
{
    ...
    public function createUserWithManagerRole()
    {
        ...
        //Create manager role with sub-role for his personal information
    ...
    public function createManagerRole($sub_role_id)
    {
        ...
        DB::table('role_permission_has_access_levels')->insert(
            [
                ['role_id' => $managerRole->id, 'permission_id' => (Permission::where('name', 'view employee firstname')->pluck('id')->first()), 'access_level_id' => 1],
                ['role_id' => $managerRole->id, 'permission_id' => (Permission::where('name', 'view employee lastname')->pluck('id')->first()), 'access_level_id' => 1]
            ]
        );
     ...
     * @return void
     */
    public function test_admin_can_view_employee_information_edit_page_for_all_employees()
    {
        ...
        $response = $this->actingAs($admin)
            ->get('en/employee/edit/' . $employee['employee']->id);
        $response->assertStatus(200);
        //Access Manager Information Edit Page
        $response = $this->actingAs($admin)
            ->get('en/employee/edit/' . $manager['manager']->id);
        $response->assertStatus(200);
        $this->assertTrue($admin->forceDelete());
        $this->assertTrue($manager['employee']->forceDelete());
        $this->assertTrue($manager['manager']->forceDelete());
        $this->assertTrue($manager['managerRole']->forceDelete());
        $this->assertTrue($manager['employeeRole']->forceDelete());
        $this->assertTrue($employee['employee']->forceDelete());
        $this->assertTrue($employee['role']->forceDelete());
    }
    /**
     *
     * @return void
     */
    public function test_employee_can_view_his_own_information_edit_page_only()
    {
        ...
        $response = $this->actingAs($employee1['employee'])
            ->get('en/employee/edit/' . $employee1['employee']->id);
        $response->assertStatus(200);
        $response = $this->actingAs($employee1['employee'])
            ->get('en/employee/edit/' . $employee2['employee']->id);
        $response->assertStatus(403);
        $this->assertTrue($employee1['employee']->forceDelete());
        $this->assertTrue($employee1['role']->forceDelete());
        $this->assertTrue($employee2['employee']->forceDelete());
        $this->assertTrue($employee2['role']->forceDelete());
    }
     ...
     * @return void
     */
    public function test_manager_can_view_his_own_and_his_direct_or_indirect_employees_edit_page_only()
    {
        ...
        $response = $this->actingAs($manager['manager'])
            ->get('en/employee/edit/' . $manager['employee']->id);
        $response->assertStatus(200);
        $response = $this->actingAs($manager['manager'])
            ->get('en/employee/edit/' . $employee['employee']->id);
        $response->assertStatus(403);
