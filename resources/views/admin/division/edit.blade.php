...
<div class="row">
    <div class="col-lg-12">
        <div class="card card-outline-info">
            <form id="division-form" action="@if(isset($locale)){{url($locale.'/divisions',$division->id)}} @else {{url('en/divisions',$division->id)}} @endif" method="post">
                    ...
                    <div class="col-md-6 pl-3">
                        <div class="form-group">
                            ...
                            <input type="text" name="division_name"
                                    value="{{old('division-name',$division->name)}}"
                    ...
                    <div class="col-md-6 pr-3">
                        <div class="form-group">
                            ...
                            <select name="status" class="form-control">
                                <option value="1"
                                        @if($division->status == 1) selected @endif>{{__('language.Active')}}
                                </option>
                                <option value="0"
                                        @if($division->status == 0) selected @endif>{{__('language.InActive')}}
                                </option>
                            </select>
                        </div>
                    </div>
                ...
                <div class="col-12 d-flex flex-sm-row flex-column mt-2 mb-2">
                    <button  type="submit" class="btn btn-primary mb-1 mb-sm-0 mr-0 ml-1 mr-sm-1 waves-effect waves-float waves-light">{{__('language.Update')}} {{__('language.Division')}}</button>
