...
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <form id="handbook-form" action="@if(isset($locale)){{route('chapter.store', [$locale])}} @else {{route('chapter.store', ['en'])}} @endif" method="post" enctype="multipart/form-data">
                {{ csrf_field() }}
                    <div class="row">
                        <div class="form-group col-md-6">
                            <label class="control-label">{{__('language.Chapter Name')}}<span class="text-danger">*</span></label>
                            <input type="text" class="form-control" name="chapter_name" value="{{ old('chapter_name') }}" placeholder="{{__('language.Enter')}} {{__('language.Chapter Name')}}" autofocus>
                        </div>
                        <div class="form-group col-md-6">
                            <label class="control-label">{{__('language.Page Title')}}<span class="text-danger">*</span></label>
                            <input type="text" name="page_title" value="{{ old('page_title') }}" class="form-control" placeholder="{{__('language.Enter')}} {{__('language.Page Title')}}">
                        </div>
                    ...
                    <div class="d-flex">
                        <button type="button" onclick="editor();" class="btn btn-primary waves-effect waves-float waves-light" data-toggle="tooltip" data-original-title="Create Chapter"><span class="d-lg-none d-md-none d-sm-inline"><i data-feather="check-circle"></i></span><span class="d-none d-lg-inline d-md-inline d-sm-none">{{__('language.Create')}}</span></button>
