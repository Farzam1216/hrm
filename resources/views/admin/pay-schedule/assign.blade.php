...
@section('title', 'Assign Pay Schedule')
        ...
        <div class="card">
            <div class="card-header border-bottom pt-1 pb-1">
                <div class="head-label">
                    <h6 class="mb-0">{{ucwords($pay_schedule->name)}}</h6>
                </div>
            </div>
            <div class="card-body pt-1">
                                ...
                                <div class="card-body">
                                    <ul class="list-group" id="employee-list">
                                        ...
                                        @foreach($employees as $employee)
                                            @if(!$employee->isAdmin())
                                                @if($employee->assignedPaySchedule == null || isset($employee->assignedPaySchedule->pay_schedule_id) && $employee->assignedPaySchedule->pay_schedule_id != $pay_schedule->id)
                                                    <li class="list-group-item" value="{{$employee->id}}">{{$employee->full_name}} @if(isset($employee->assignedPaySchedule->paySchedule)) ( {{ucwords($employee->assignedPaySchedule->paySchedule->name)}} ) @php if($employeeIds){$employeeIds = $employeeIds.','.$employee->id;} else {$employeeIds = $employee->id;} @endphp @endif</li>
                                                @endif
                                            @endif
                                        @endforeach
                                    </ul>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <ul class="list-group" id="assign-list">
                                        ...
                                        @foreach($employees as $employee)
                                            @if(!$employee->isAdmin())
                                                @if(isset($employee->assignedPaySchedule->pay_schedule_id) && $employee->assignedPaySchedule->pay_schedule_id == $pay_schedule->id)
                ...
                <div class="d-flex">
                    <button type="button" class="btn btn-primary waves-effect waves-float waves-light" id="btnSave"><span class="d-lg-none d-md-none d-sm-block"><i data-feather='check-circle'></i></span><span class="d-none d-lg-inline d-md-inline d-sm-none">{{__('language.Assign')}} {{__('language.Pay Schedule')}}</span></button>
                    <button type="button" onclick="window.location.href='@if(isset($locale)) {{route('pay-schedule.index', [$locale])}} @else {{route('pay-schedule.index', ['en'])}} @endif'" class="btn btn-outline-warning waves-effect waves-float waves-light ml-1"><span class="d-lg-none d-md-none d-sm-block"><i data-feather='x-circle'></i></span><span class="d-none d-lg-inline d-md-inline d-sm-none">{{__('language.Cancel')}}</span></button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel1">Assign Pay Schedule Confirmation</h4>
            ...
            <div class="modal-body mt-1">
                ...
                <h5>This action will update already assigned Pay Schedule of employee. Do you still want to perform this action?</h5>
            </div>
    ...
    $(document).ready(function() {
        ...
        $('#btnSave').click(function() {
            ...
            var data = {
                ...
                pay_schedule_id: {!! $pay_schedule->id !!}
            };
            ...
            if ($("#confirmation").val() == 'yes' || check == false) {
                $.ajax({
                    url: '{{ route('pay-schedule.submitAssignment', ['+en+', $pay_schedule->id]) }}',
                    headers: {
                    ...
                    success: function(data) {
                        window.location.href = '/en/pay-schedule';
                    }
                });
            }
        });
