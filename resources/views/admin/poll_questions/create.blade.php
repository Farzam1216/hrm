  ...
  <div class="card">
    <div class="card-content">
      <div class="card-body ">
        ...
        <form method="post" id="poll-questions" action="@if(isset($locale)){{route('polls.questions.store',[$locale,$poll->id])}} @else {{route('polls.questions.store',['en',$poll->id])}} @endif">
          {{csrf_field()}}
          <div class="row">
          ...
          <div class="row">
            <div class="col-12 ">
              ...
              <div class="form-group">
                ...
                <input type="text" name="title" required class="form-control" placeholder="Question" data-validation-required-message="Question title is required">
              </div>
            </div>
          </div>
          <div class="divider divider-right">
            <div class="divider-text"> <button type="button" class="btn btn-outline-primary waves-effect" data-toggle="modal" data-target="#addAnswer"><i data-feather="plus"></i> {{__('language.Add')}} {{__('language.Option')}}</button></div>
          </div>
          ...
          <div class="row">
            <div class="col-12 col-sm-6">
              <h5 class="mb-1 mt-2 mt-sm-0">{{__('language.Options')}}</h5>
            </div>
          </div>
          <div class="row">
            <div class="col-12">
                ...
                <div class="modal fade" id="deleteAnswer" tabindex="-1" role="dialog" aria-labelledby="deleteAnswerTitle" aria-hidden="true">
                  <div class="modal-dialog modal-dialog-centered modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        <button type="button" id="" value="" onclick="deleteQuestionOption(this)" class="btn btn-danger delete-btn">{{__('language.Yes')}}</button>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div class="col-12 d-flex flex-sm-row flex-column justify-content-end mt-1">
            <button type="submit" style="color: white;" class="btn btn-primary glow mb-1 mb-sm-0 mr-0 mr-sm-1">{{__('language.Add')}} {{__('language.Question')}}
            </button>
            <button type="button" onclick="window.location.href='@if(isset($locale)){{route('polls.questions.index',[$locale,$poll->id])}} @else {{route('polls.questions.index',['en',$poll->id])}} @endif'" class="btn btn-outline-warning">Cancel
            </button>
          </div>
