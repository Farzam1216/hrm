        ...
        <div class="card">
            <div class="card-header border-bottom pb-1 pt-1">
                ...
                <div class="dt-action-buttons text-right dt-buttons flex-wrap d-inline-flex">
                    <a href="@if(isset($locale)){{url($locale.'/question-types/create')}} @else {{url('en/question-types/create')}} @endif"
            ...
            <div class="card-datatable table-responsive pt-0" style="padding: 15px;">
                <table class="dt-simple-header table dataTable dtr-column">
                    <thead class="head-light">
                        <tr>
                            <th>#</th>
                            <th> {{__('language.Question Type')}}</th>
                        ...
                        @foreach($questionTypes as $key => $questionType)
                            <tr>
                                ...
                                <td class="text-nowrap">
                                    ...
                                    href="@if(isset($locale)){{url($locale.'/question-types/'.$questionType->id.'/edit')}}
                                    @else {{url('en/question-types/'.$questionType->id  .'/edit')}} @endif">
                                     <i data-feather="edit-2" class="mr-40"> </i></a>
                                    
                                    <a class="btn btn-sm btn-clean btn-icon btn-icon-md" data-toggle="modal"
                                        data-target="#confirm-delete{{  $questionType->id }}"
                            ...
                            <div class="modal fade" id="confirm-delete{{ $questionType->id }}" tabindex="-1"
                                role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
                               <div class="modal-dialog">
                                   <div class="modal-content">
                                       <div class="modal-header">
                                            <h5>Delete Question Type</h5>
                                           <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                               <span aria-hidden="true">&times;</span>
                                           </button>
                                       </div>
                                       <form action="@if(isset($locale)){{url($locale.'/question-types/'.$questionType->id)}} @else {{url('/question-types/'.$questionType->id)}} @endif"
                                             method="post">
                                           <input name="_method" type="hidden" value="DELETE">
                                           {{ csrf_field() }}
                                           <div class="modal-body">
                                               <h5>Are you sure you want to delete "{{$questionType->type}}" question type? </h5>
                                           </div>
