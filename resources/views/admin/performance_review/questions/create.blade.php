...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <form id="question-form" action="@if(isset($locale)){{route('questions.store', [$locale])}} @else {{route('questions.store', ['en'])}} @endif" method="post">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="control-label">{{__('language.Field Type')}}</label><span class="text-danger"> *</span>
                                <select class="form-control" type="text" id="field_type" name="field_type" value="{{ old('field_type') }}" autofocus>
                                    <option value="">Select Option</option>
                                    <option value="small text field" @if(old('field_type') == "small text field") selected @endif>Small Text Field</option>
                                    <option value="long text field" @if(old('field_type') == "long text field") selected @endif>Long Text Field</option>
                                    <option value="multiple choice button" @if(old('field_type') == "multiple choice button") selected @endif>Multiple Choice Button</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="placement" name="placement" value="{{ old('placement') }}" placeholder="Enter number from 0-9">
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-group">
                                <label class="control-label">{{__('language.Question')}}</label><span class="text-danger"> *</span>
                                <textarea class="form-control" id="question" name="question" placeholder="{{__('language.Enter')}} {{__('language.Question')}} {{__('language.Here')}}">{{ old('question') }}</textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <button type="submit" class="btn btn-primary mr-1">
                            ...
                            <span class="d-none d-lg-inline d-md-inline d-sm-inline">{{__('language.Add')}} {{__('language.Question')}}</span>
                        </button>
