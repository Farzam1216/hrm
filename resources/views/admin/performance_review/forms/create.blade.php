...
<div class="row">
    <div class="col-lg-12">
        <div class="card">
            <div class="card-body">
                <form id="form" action="@if(isset($locale)){{route('forms.store', [$locale])}} @else {{route('forms.store', ['en'])}} @endif" method="post">
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="control-label">{{__('language.Name')}}</label><span class="text-danger"> *</span>
                                <input type="text" class="form-control" id="name" name="name" value="{{ old('name') }}" placeholder="Enter Questionnaire Form Name" autofocus>
                            </div>
                        </div>
                    </div>
                    <div class="row mt-2">
                    ...
                    <div class="row">
                        @foreach($questions as $key => $question)
                            <div class="col-md-6 pt-1">
                                <div class="custom-control custom-checkbox">
                                    <input type="checkbox" class="custom-control-input checkboxes" id="question{{$question->id}}" name="questions[]" value="{{$question->id}}">
                                    <label class="custom-control-label" for="question{{$question->id}}">Q-{{$key+1}}: {{$question->question}}</label>
                                </div>
                            </div>
                        @endforeach
                        ...
                        <div class="col-12 pt-2 hidden" id="checkboxes-error">
                         ...
                         <div class="col-12 pt-1 @if($questions != '[]') hidden @endif" id="questions-error">
                            <label class="error">Please add questions and try again</label>
                        </div>
                    </div>
                    ...
                    <div>
                        <button type="button" class="btn btn-primary mr-1" onclick='validate({!! $questions !!});'>
                            <span class=" d-lg-none d-md-none d-sm-none"><i data-feather="check-circle"></i></span>
                            <span class="d-none d-lg-inline d-md-inline d-sm-inline">{{__('language.Add')}} {{__('language.Form')}}</span>
                        </button>
...
<script>
    ...
    function validate(questions)
    {
        if (questions.length <= 0) {
            $("#questions-error").removeClass('hidden');
        }
        ...
        if (questions.length > 0) {
            ...
            var check = false;
            $("#questions-error").addClass('hidden');
