...
@section('title','New Custom Access Level')
@section('heading')
...
<div class="row">
    <div class="container-fluid">
        <form id="customRole" action="@if(isset($locale)){{url($locale.'/access-level/custom')}} @else {{url('en/access-level/custom')}} @endif" method="post" enctype="multipart/form-data">
                ...
                <div class="card card-primary card-tabs">
                    <div class="card-header p-0 pt-1 pl-1 pr-1">
                        <ul class="nav nav-tabs nav-justified" id="custom-tabs-one-tab" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link step" id="what-does-tab" aria-controls="what-does" aria-selected="false">{{__('language.What this access level can do')}}?</a>
                            </li>
                            <li class="nav-item">
                        ...
                        <div class="tab-content " id="custom-tabs-one-tabContent">
                            <div class="tab-pane fade tab show active" id="basic-info" role="tabpanel" aria-labelledby="basic-info-tab">
                                <div class="form-group col-md-12">
                                    <br />
                                    <p>
                                        Custom Access Levels can be configured to allow Employees to see
                                        and/or edit information for some or all employees in your company.
                                    </p>
                                    <hr>
                                    <div class="form-group ">
                                        <label class="control-label">Access Level Name
                                            <span class="text-danger">*</span></label>
                                        <input class="form-control col-md-6" maxlength="100" type="text" required="required" name="name" id="access_level_name" />
                            ...
                            <div class="tab-pane  tab" id="what-does" role="tabpanel" aria-labelledby="what-does-tab">
                                <div class="col-md-12">
                                    <br />
                                    <p><b>What Can People with this Access Level Do?</b></p>
                                    <p><b>Select an area to customize access.</b></p>
                                    <hr>
                                    <div class="row">
                                        @foreach($manageFullPermissions as $fieldName => $permissionActions)
                                        <div class="col-md-3 col-sm-12">
                                            <div class="nav flex-column nav-tabs h-100" id="vert-tabs-tab" role="tablist" aria-orientation="vertical">
                                                <a class="nav-link vert-tabs" id="vert-tabs-{{$fieldName}}-tab" data-toggle="pill" name="fullPermissions[]" href="#vert-tabs-{{$fieldName}}" role="tab" aria-controls="vert-tabs-{{$fieldName}}" aria-selected="true">
                                        ...
                                        <div class="col-md-12">
                                            <div class="tab-content" id="vert-tabs-tabContent">
                                                @foreach($manageFullPermissions as $fieldName => $fullPermission)
                                                <div class="tab-pane text-left fade show" id="vert-tabs-{{$fieldName}}" role="tabpanel" aria-labelledby="vert-tabs-{{$fieldName}}-tab">
                                                    <h4>{{ucwords (str_replace('_',' ',$fieldName))}}</h4>
                                                    <hr>
                                                    <div class="">
                                                        <!-- checkbox -->
                                                        <div class="form-group mt-2">
                                                            @foreach($fullPermission as $key => $permissions)
                                                            @foreach($permissions as $field => $permission)
                                                            <div class="custom-control custom-checkbox mt-1 ml-1">
                                                                <input class="custom-control-input" type="checkbox" id="{{$key}}" value="{{$permission}}" name="fullPermissions[]">
                                                                <label for="{{$key}}" class="custom-control-label">{{ucwords (str_replace('_', ' ',$key))}}</label>
                                                            </div>
                                                            @endforeach
                                                            @endforeach
                                                        </div>
                                        ...
                                        <div class="">
                                            <div class="tab-content" id="custom-tabs-one-tabContent">
                                                <div class="tab-pane fade show active" id="custom-tabs-one-home" role="tabpanel" aria-labelledby="custom-tabs-one-home-tab">
                                                    <div class="form-group row " id="access_level_row">
                                                        <label class="col-form-label pl-1">
                                                            <i data-feather='lock'></i>&nbsp; <b>This Access Level can access the information below for</b>
                                                        </label>
                                                        <div class="pl-1">
                                                            <select class="form-control mr-1" name="access_level" onchange="getEmployees({{$employees}},this.value)">
                                                                <option value="4">All Employees
                                                                </option>
                                                                <option value="1">Direct Employees
                                                                </option>
                                                                <option value="2">Direct + Indirect
                                                                    Employees
                                                                </option>
                                                                <option value="3">Only Some Employees..
                                                                </option>
                                                            </select>
                                                        </div>
                                                        <div id="some_employees" class="ml-1 mt-1" style="column-count: 3;">
                                                    ...
                                                    <div class="">
                                                        <ul class="nav nav-tabs" id="custom-content-below-tab" role="tablist">
                                                            ...
                                                            <li class="nav-item col-lg-2 col-md-2 col-sm-12">
                                                                <a class="nav-link @if($count==0) active @endif" id="employee-{{$category}}-info-tab" data-toggle="pill" href="#employee-{{$category}}-info" role="tab" aria-controls="employee-{{$category}}-info" aria-selected="true">
                                                        ...
                                                        <div class="tab-content" id="custom-content-below-tabContent">
                                                            ...
                                                            <div class="tab-pane fade show @if($count==0) active @endif" id="employee-{{$category}}-info" role="tabpanel" aria-labelledby="employee-{{$category}}-info-tab">
                                                                ...
                                                                @if($group == "checkbox" && $category == "attendance")
                                                                    <div class="custom-control custom-control-primary custom-checkbox">
                                                                        <input type="checkbox" name="customPermissions[]" value="view employee_attendance" class="custom-control-input requestAttendance" id="colorCheck3">
                                                                        <label class=" custom-control-label" for="colorCheck3">
                                                                            Can View Employee Attendance
                                                                        </label>
                                                                    </div>
                                                                    <div class="col-12 attendanceList" style="display:none; width:  270px; border: 1px solid grey; border-radius: 5px; margin-left: 30px; margin-top: 2px; padding: 2px;">
                                                                        @foreach($permission as $permissionLabel=>$permissionValue)
                                                                        <div class="custom-control custom-checkbox">
                                                                            <input class="custom-control-input myCheckbox" type="checkbox" id="{{$permissionLabel}}" value="edit employee_attendance" name="customPermissions[]">
                                                                            <label for="{{$permissionLabel}}" class="custom-control-label">Can Mark Employee Attendance</label>
                                                                        </div>
                                                                        @endforeach
                                                                    </div>
                                                                @endif
                                                                ...
                                                                <div class="row col-md-12 pl-2">
                                                                    ...
                                                                    <div class="row col-lg-9 col-md-12 col-sm-12">
                                                                        <div class="mt-2 flex-fill col-md-7 col-sm-4">
                                                                            <input type="hidden" id="{{$group}}{{$fieldName}}SelectedValue" name="customPermissions[]">
                                                                        ...
                                                                        <div class="mt-1 flex-fill col-md-5 col-sm-6">
                                                                            <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                                                                <label class="btn btn-primary active p-1 {{$category}}_no_access_button">
                                                                                    <i data-feather='lock'></i>
                                                                                    <input type="radio" class="{{$category}}_no_access" id="{{$group}}{{$fieldName}}no_access" access="no_access" permission="" value="{{$group}}{{$fieldName}}" checked onclick="getPermissionValue(this);">
                                                                                </label>
                                                                                @foreach($permissionActions as $action)
                                                                                @if(strpos($action, 'view') !== false)
                                                                                <label class="btn btn-primary p-1 {{$category}}_view_button">
                                                                                    <i data-feather='eye'></i>
                                                                                    <input type="radio" class="{{$category}}_view" id="{{$group}}{{$fieldName}}view" access="view" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onclick="getPermissionValue(this);">
                                                                                </label>
                                                                                @elseif(strpos($action, 'edit ') !== false)
                                                                                <label class="btn btn-primary p-1 {$category}}_edit_button">
                                                                                    <i data-feather='edit-2'></i>
                                                                                    <input type="radio" id="{{$group}}{{$fieldName}}edit" access="edit" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onclick="getPermissionValue(this);">
                                                                                </label>
                                                                                @elseif(strpos($action, 'edit_with_approval') !== false)
                                                                                <label class="btn btn-primary p-1 {$category}}_edit_with_approval_button">
                                                                                    <i data-feather='user-check'></i>
                                                                                    <input type="radio" class="{{$category}}_edit_with_approval" id="{{$group}}{{$fieldName}}edit_with_approval" access="edit_with_approval" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onclick="getPermissionValue(this);">
                                                                                </label>
                                                                                @endif
                                                                                @endforeach
                                                                            </div>
                                                                ...
                                                                @endforeach
                                                                <!--Skip the permission with checkbox:see company payment for benefit -->
                                                                ...
                                                                <div class="collapse" id="{{$group}}{{$category}}Tab">
                                                                    ...
                                                                    <div class="row col-md-12 pl-2">
                                                                        ...
                                                                        <div class="row col-lg-9 col-md-12 col-sm-12">
                                                                            <div class="mt-2 flex-fill col-md-7 col-sm-4">
                                                                                <input type="hidden" id="{{$group}}{{$fieldName}}SelectedValue" name="customPermissions[]">
                                                                                <p id="restrictToNoAccess" class="{{$category}}_no_access_label restrictToNoAccess {{$group}}{{$fieldName}}no_access"> <i data-feather='lock'></i> No Access</p>
                                                                                <p id="restrictToViewOnly" class="{{$category}}_view_label restrictToViewOnly {{$group}}{{$fieldName}}view"> <i data-feather='eye'></i> View Only</p>
                                                                                <p id="restrictToEdit" class="{{$category}}_edit_label restrictToEdit {{$group}}{{$fieldName}}edit"> <i data-feather='edit-2'></i> Edit</p>
                                                                                <p id="restrictToEditWithApproval" class="{{$category}}_edit_with_approval_label restrictToEditWithApproval {{$group}}{{$fieldName}}edit_with_approval"> <i data-feather='user-check'></i> Edit - Approval Required</p>
                                                                            </div>
                                                                            <div class="mt-1 flex-fill col-md-5 col-sm-6">
                                                                                <div class="btn-group btn-group-toggle" data-toggle="buttons">
                                                                                    <label class="btn btn-primary active p-1 {{$category}}_no_access_button">
                                                                                        <i data-feather='lock'></i>
                                                                                        <input type="radio" class="{{$category}}_no_access" id="{{$group}}{{$fieldName}}no_access" value="{{$group}}{{$fieldName}}" access="no_access" permission="" checked onchange="getPermissionValue(this);">
                                                                                    </label>
                                                                                    @foreach($permissionActions as $action)
                                                                                    @if(strpos($action, 'view') !== false)
                                                                                    <label class="btn btn-primary p-1 {{$category}}_view_button">
                                                                                        <i data-feather='eye'></i>
                                                                                        <input type="radio" class="{{$category}}_view" id="{{$group}}{{$fieldName}}view" access="view" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onchange="getPermissionValue(this);">
                                                                                    </label>
                                                                                    @elseif(strpos($action, 'edit ') !== false)
                                                                                    <label class="btn btn-primary p-1 {{$category}}_edit_button">
                                                                                        <i data-feather='edit-2'></i>
                                                                                        <input type="radio" class="{{$category}}_edit" id="{{$group}}{{$fieldName}}edit" access="edit" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onchange="getPermissionValue(this);">
                                                                                    </label>
                                                                                    @elseif(strpos($action, 'edit_with_approval') !== false)
                                                                                    <label class="btn btn-primary p-1 {{$category}}_edit_with_approval_button">
                                                                                        <i data-feather='user-check'></i>
                                                                                        <input type="radio" class="{{$category}}_edit_with_approval" id="{{$group}}{{$fieldName}}edit_with_approval" access="edit_with_approval" permission="{{$action}}" value="{{$group}}{{$fieldName}}" onchange="getPermissionValue(this);">
                                                                                    </label>
                                                                                    @endif
                                                                                    @endforeach
                                                                                </div>
                                                ...
                                                <div class="tab-pane fade" id="custom-tabs-one-profile" role="tabpanel" aria-labelledby="custom-tabs-one-profile-tab">
                                                    ...
                                                    <div class="form-check mt-2 ml-1">
                                                        <input class="form-check-input" type="radio" value="yes" name="hasEmployeeRole" id="yesRadiobtn" checked>
                                                        <label class="form-check-label" for="yesRadiobtn">
                                                            Yes, Allow Access
                                                        </label>
                                                    </div>
                                                    <div class="form-check mt-1 ml-1">
                                                        <input class="form-check-input" type="radio" value="no" name="hasEmployeeRole" id="noRadiobtn">
                                                    ...
                                                    <div id="toggleAccessWithRadioBtn" class="mt-1 ml-1">
                                                        ...
                                                        <select class="form-control col-md-3 pr-2" id="selectLevel" name="employeeRole">
                                                            ...
                                                            @endforeach
                                                            <option value="fullAccessOption">
                                                                Full Access
                                                                <small>Allows them to see the
                                                                    same info about themselves
                                                                    as they can see about
                                                                    others.
                                                                </small>
                                                            </option>
                                                        </select>
                                                    ...
                                                    <div class="row ml-1">
                                                        <div class="custom-control custom-checkbox" id="toggleWithFullAccessOption">
                                                            <input class="custom-control-input" type="checkbox" id="canRequestCb">
...
<script>
    ...
    function submitForm()
    {
        ...
        document.getElementById("customRole").submit();
    ...
    $('#yesRadiobtn').click(function() {
        ...
        if ($("#selectLevel option:selected").val() == "fullAccessOption") {
    ...
    $("#canEditBasicInfo").change(function() {
        $('#restrictToNoAccess').hide();
        $('#restrictToViewOnly').hide();
        $('#restrictToEdit').show();
    });

    function getEmployees(employees, accessLevel) {
        if (accessLevel == 3) {
            console.log(employees, accessLevel);
            $.each(employees, function(indexOfElement, valueOfElement) {
                console.log(valueOfElement);
                $('#some_employees').append(
                    '<div class="custom-control custom-checkbox" >\n' +
                    '  <input type="checkbox" class="custom-control-input" id="specific_employees' + valueOfElement.id + '" value="' + valueOfElement.id + '" name="specific_employees[]">\n' +
                    '  <label class="custom-control-label" for="specific_employees' + valueOfElement.id + '">' + valueOfElement.firstname + '</label>\n' +
                    '</div>'
                );
            });
        } else {
            $('#some_employees').empty();
        }
    }

    $('.restrictToViewOnly').hide();
    $('.restrictToEdit').hide();
    $('.restrictToEditWithApproval').hide();
    ...
    $("#vert-tabs-employee-tab").addClass('active');
    $("#vert-tabs-employee").addClass('active');

    $("#vert-tabs-hiring-tab").click(function() {
        $('#vert-tabs-employee-tab').removeClass('active');
        $('#vert-tabs-settings-tab').removeClass('active');

        $("#vert-tabs-employee").removeClass('active');
        $("#vert-tabs-settings").removeClass('active');
    });
    $("#vert-tabs-settings-tab").click(function() {
        $('#vert-tabs-employee-tab').removeClass('active');
        $('#vert-tabs-hiring-tab').removeClass('active');

        $("#vert-tabs-employee").removeClass('active');
        $("#vert-tabs-hiring").removeClass('active');
    });

    $("#vert-tabs-employee-tab").click(function() {
        $('#vert-tabs-settings-tab').removeClass('active');
        $('#vert-tabs-hiring-tab').removeClass('active');
