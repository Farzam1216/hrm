...
<nav class="header-navbar navbar-expand-lg navbar navbar-fixed align-items-center navbar-shadow navbar-brand-center {{ $configData['navbarColor'] }}" data-nav="brand-center">
  ...
  <nav class="header-navbar navbar navbar-expand-lg align-items-center {{ $configData['navbarClass'] }} navbar-light navbar-shadow {{ $configData['navbarColor'] }}">
    ...
    <div class="navbar-container d-flex content">
      ...
      <ul class="nav navbar-nav align-items-center ml-auto">
        ...
        <li class="nav-item dropdown dropdown-notification mr-25"><a class="nav-link" href="javascript:void(0);" data-toggle="dropdown"><i class="ficon" data-feather="bell"></i><span class="badge badge-pill badge-danger badge-up">{{Auth::user()->unreadNotifications->count()}}</span></a>
          <ul class="dropdown-menu dropdown-menu-media dropdown-menu-right">
            <li class="dropdown-menu-header">
              <div class="dropdown-header d-flex">
            ...
            <li class="scrollable-container media-list">
              <!-- Getting Notifications of logged in user -->
              @foreach(Auth::user()->unreadNotifications as $notification)
              <a class="d-flex" href="javascript:void(0)">
                <div class="media d-flex align-items-start">
                  <div class="media-body">
                    <a href="@if(isset($notification->data['url']))  @if(isset($locale)){{url($locale.$notification->data['url'])}} @else {{url('en'.$notification->data['url'])}} @endif @endif" id="unreadNotification"  getNotificationID="{{ $notification['id'] }}" getNotificationReadStatus="{{ $notification->read_at }}"><p class="media-heading"><span class="font-weight-bolder">@if(isset($notification->data['title'])){{$notification->data['title']}}@endif</p><small class="notification-text">@if(isset($notification->data['message'])){{$notification->data['message']}}@endif</small></a>
                  </div>
                </div>
              </a>
              @endforeach
            </li>
            <li class="dropdown-menu-footer"><a class="btn btn-primary btn-block" onclick="readAllNotifications()" href="" >Read all notifications</a></li>
          </ul>
        </li>
        <li class="nav-item dropdown dropdown-user">
          <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            <div class="user-nav d-sm-flex d-none">
              <!-- this class can be used for naming the user name styling class="user-name"  -->
              <span class="font-weight-bolder">{{Auth::user()->firstname}}</span>
            </div>
            <span class="avatar">
              <img class="round" src="{{asset(Auth::user()->picture)}}" onerror="this.src ='{{asset('asset/media/users/default3.png')}}';" alt="avatar" height="40" width="40">
              <span class="avatar-status-online"></span>
            </span>
          </a>
          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-user">
            <a class="dropdown-item" href="@if(isset($locale)){{url($locale.'/personal-profile/')}} @else {{url('en/personal-profile/')}} @endif">
              <i class="mr-50" data-feather="user"></i> {{__('language.Edit Profile')}}
            </a>
  ...
  <script>
    $("#unreadNotification").on('click', function(){
      ...
      var status = $(this).attr('getNotificationReadStatus');
    ...
    function readAllNotifications(){
      $.ajax({
        url: '{!! URL('/en/read-all-notifications') !!}',
        type: 'get',
        dataType: 'json',
        success: function (data) {
          location.reload();
